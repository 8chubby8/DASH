...
- Edit mode — binding visual feedback **(→ 1.3.10)** — element boxes in the ruler show a red tint on whichever edge is currently bound to a zone edge or an adjacent element's edge. Divider arrow visual feedback to be defined once it is clear whether snap detents will be reintroduced — if snap returns, arrows turn red at exact snap points; if not, an alternative visual for divider state will be designed. This replaces any text-based position labelling with a purely visual convention consistent throughout the ruler
