...
*Cleanup before 1.5.x — a short 1.4.x refinement pass folds in niceties deferred during the build. Known so far: the **USB one-time permission grant** — a `USB_DEVICE_ATTACHED` intent-filter + `device_filter.xml` (standard serial VIDs) so a device is authorised once with "use by default" rather than re-prompting on every replug. Android forbids zero-consent for a sideloaded app, so one-and-done is the floor on Bronze; silent anyway on the system-app production board.*
