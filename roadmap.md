...
- Edit mode — ruler and interaction model **(→ 1.3.7)** — the original plan for direct long-press-drag on the bar has been superseded by a design review. The new model: when edit mode is entered, a control strip appears a finger-width's gap from the bar on its inner side (below if top-docked, above if bottom-docked). The bar itself is never touched directly during editing — all interaction happens in the ruler. Zone dividers are represented in the ruler as arrow markers pointing back to their bar position. Elements are represented as footprint-sized boxes visible only in edit mode — no permanent boundary box exists in normal operation. The whole-bar border/tint treatment from 1.3.6 is removed — the ruler's presence alone is the unambiguous signal that edit mode is active; a second signal repeating the same information is redundant
- Edit mode — Save, Cancel, and edit workspace **(→ 1.3.8)** — replace the DONE button with separate SAVE and CANCEL actions. Both live in the main content area, which becomes the edit workspace for the duration of editing — consistent with how the settings panel already takes over the full screen. On entry, snapshot the current bar config. SAVE commits the in-memory edited state to DataStore. CANCEL discards all changes and restores the bar exactly to the snapshot, with no write
- Edit mode — zone configuration relocation **(→ 1.3.9)** — move the zone count control (1/2/3 buttons) from the main Settings panel's System Bar section into the edit workspace alongside the ruler. Remove zone distribution preset buttons entirely — they predate divider dragging and are redundant now that dragging a divider to a detent snap achieves the same result more directly. The main Settings panel retains only the EDIT BAR LAYOUT entry point going forward
- Edit mode — binding visual feedback **(→ 1.3.10)** — element boxes in the ruler show a red tint on whichever edge is currently bound to a zone edge or an adjacent element's edge. Divider arrows turn red specifically when settled at a snap point, not merely when nearby — distinguishing a deliberate alignment from one that is merely close. This replaces any text-based position labelling with a purely visual convention, consistent for both dividers and elements throughout the ruler
